...
import path from 'path'; // we will use path module to work with file and directory paths in a way that is compatible across different operating systems
...
const loadLinks = async () => { // this function will read the links.json file and return the data in JSON format. This function will be used to load all the existing links from the links.json file.
    try {
        ...
        return JSON.parse(data); // Used to convert a JSON string into a JavaScript object.
    }
    catch(err) {
...
const saveLinks = async (links) => { // This function will be used to save the new link to the links.json file.
    await writeFile(path.join("data", 'links.json'), JSON.stringify(links)); // we will write the updated links object to the "links.json" file by converting JSON formate.
}
...
const server = createServer(async (req, res) => {
    ...
    if(req.method === 'GET') { // if request is hitted by GET method
        // ✅ Serve HTML file in NodeJS (if the user is hits the root URL (Homepage))
        if(req.url === '/') { 
            try{ // jo data male to... 
        ...
        // ✅ before requesting from fronted, we have to create a backend API in app.js file which will return all the shortened URLs stored in "data -> links.json" file, and then we will fetch that API in frontend and display the data in frontend.
            ...
            return res.end(JSON.stringify(links)); // Opposite of parse() method, it is used to convert a JavaScript object into a JSON string. We will end the response by sending the data of links which we have loaded from the links.json file in JSON format.
        }
        ...
        // ✅ when we hit the shortCode in URL then it should redirect to original URL
            ...
            if(links[shortCode]) { // we will check if the shortCode exists in the links object, then we will redirect to the original URL.
    ...
    // Lec-35 :- How to store all URL's & its shortCode in "data -> links.json" in NodeJS
        ...
        if(req.url === '/shorten') { // if the user is hits the /shorten URL, then we will get all the data till server sends the data.
            ...
            let body = ''; // Form data which written by the user in frontend -> store in "body" variable. We will get this data in chunks as the server sends the data in chunks, so we will listen to the "data" event on the request object to get the data in chunks and we will concatenate it to the "body" variable until we receive all the data from the server.
            ...
            req.on('end', async () => { // Now, there is no more data left to receive, so the "end" event will be triggered. 
                ...
                const { url,shortCode } = JSON.parse(body); // convert JSON string into JavaScript object and we will get the url and shortCode from the request body which is sent by the client in JSON format.
                if(!url){
